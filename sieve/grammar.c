...
EXPORTED char *parse_string(const char *s, variable_list_t *vars)
{
	...
	*variable_ref_end = '\0';
	...
	if (is_identifier(variable_ref_begin+2)) {
	    ...
	    *variable_ref_begin = '\0';
	     ...
	     */
	    if ((variable = varlist_select(vars, variable_ref_begin+2))) {
			char *temp;
    ...
    strarray_append(varlist_select(vars, VL_PARSED_STRINGS)->var,
	    test_str = strarray_join(&stringparts, ""));
